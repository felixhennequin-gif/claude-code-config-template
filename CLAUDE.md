...
- **`git reset --hard` / `git clean -fd` / `git branch -D` are intentionally excluded from the permissions allowlist.** `settings.json` lists git subcommands explicitly instead of `Bash(git:*)` — destructive rewrites require a manual run. Don't "simplify" this back to a wildcard. Note: `Bash(git checkout:*)` and `Bash(git restore:*)` are wildcards on purpose (needed for rebase conflict resolution and file-level restore). These *could* allow `git checkout -- .`, so Claude must still confirm before running anything that discards uncommitted work — see `.claude/rules/git-workflow.md`.
