...
## `pre-commit-secret-scan.sh` smoke tests

Every case below must produce the expected exit code. Patterns starting with `-` (the PEM `-----BEGIN … PRIVATE KEY-----` block) require `grep -qE -- "$P"` so `grep` does not parse them as flags — verify before any pattern addition.
...
# Run each from a temp git repo where the offending file is staged. Pattern coverage:
